...
import com.famous.covid19detector.databinding.ActivitySignUpBinding
...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun validateData() {
        ...
        password = binding.passwordRegister.toString().trim()
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()){
            ...
            binding.passwordRegister.error = "password isn't entered"

        }else if (password.length < 6){
            //password length is less than 6
            binding.passwordRegister.error = "Password must ave atleast 6 character"
        }else {
